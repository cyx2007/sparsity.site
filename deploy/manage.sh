#!/usr/bin/env bash
# Ubuntu/Linux host, run with sudo. All live state stays outside the checkout.
set -Eeuo pipefail
umask 077
...
case ${1:-help} in
  deploy)
    TARGET=${2:?Usage: deploy RELEASE_ID}
    ...
    docker build --pull -t "sparsity:$TARGET" "$SOURCE"
    ...
    docker image inspect --format '{{.Id}}' "sparsity:$TARGET" > "$ROOT/releases/$TARGET/image"
    chown 1000:1000 "$ROOT/data"
    compose "$TARGET" pull proxy
  ...
  status|logs)
    ...
    ;;
  *)
    echo 'Usage: sudo bash deploy/manage.sh deploy RELEASE | rollback [RELEASE] | backup | restore BACKUP --confirm-data-loss | status | logs'
    ;;
esac
